...
namespace RiotSwitcherMinimal
{
    public class RiotSwitcherAppContext : ApplicationContext
    {
        ...
        public RiotSwitcherAppContext(bool isStartup = false)
        {
            _isStartup = isStartup;
            _profileManager = new ProfileManager();

            // When launched at boot, Explorer may not have the system tray ready yet.
            // Wait a few seconds to give it time to initialize.
            if (_isStartup)
            {
                Thread.Sleep(8000);
            }
        ...
        private void InitializeComponent()
        {
            ...
            // Retry making the icon visible in case Explorer wasn't ready
            if (_isStartup)
            {
        ...
        private void ToggleStartup(object? sender, EventArgs e)
        {
            try
            {
                using (var key = Microsoft.Win32.Registry.CurrentUser.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", true))
                {
                    ...
                    {
                        // Quote the path and add --startup flag so the app knows it was launched at boot
                        key?.SetValue("RiotSwitcherMinimal", $"\"{Application.ExecutablePath}\" --startup");
                    }
                }
